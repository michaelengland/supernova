...
describe "Supernova::Solr::Core" do
  ...
  let(:core_url) { "http://path.to.solr:1122/solr/my_name" }
  let(:core) { Supernova::Solr::Core.new("http://path.to.solr:1122/solr", "my_name") }
  ...
  it "creates the correct core" do
    ...
    stub_request(:get, "http://path.to.solr:1122/solr/admin/cores?action=CREATE&dataDir=/path/to/data&instanceDir=/path/to/instance&name=my_name&wt=json")
      .to_return(:status => 200, :body => "", :headers => {})
    core.create(instance_path, data_path)
  end

  it "correctly queries for core status" do
    instance_path = "/path/to/instance"
    data_path = "/path/to/data"
    stub_request(:get, "http://path.to.solr:1122/solr/admin/cores?action=STATUS&core=my_name&wt=json")
      .to_return(:status => 200, :body => "", :headers => {})
    core.status
  end
  ...
  it "unloads the correct core" do
    instance_path = "/path/to/instance"
    data_path = "/path/to/data"
    stub_request(:get, "http://path.to.solr:1122/solr/admin/cores?action=UNLOAD&core=my_name&deleteIndex=true&wt=json")
    .to_return(:status => 200, :body => "", :headers => {})
    core.unload
  end
